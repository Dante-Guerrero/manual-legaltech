# Datos abiertos necesitan traduccion visual

- ID: 202605240079
- Fuente: Schrarstzhaupt et al. (2024)
- Archivo fuente: [schrarstzhaupt-2024-paineis-de-monitoramento-interativos-da-pandemia-de-covid-19-no-mundo-com-o-uso-de-dados-abertos-ant.md](/Users/dante/github/manual-legaltech/+/y/papers/markdown/schrarstzhaupt-2024-paineis-de-monitoramento-interativos-da-pandemia-de-covid-19-no-mundo-com-o-uso-de-dados-abertos-ant.md)
- Capitulos potenciales: 3, 4
- Temas: datos-abiertos,visualizacion,paneles

## Idea

Los datos abiertos adquieren mayor utilidad publica cuando se traducen en visualizaciones y paneles comprensibles.

## Utilidad para el manual

Sirve para justificar ejercicios vinculados con datos, graficos o comunicacion de informacion.

## Posibles inserciones

- cierre del capitulo 3 sobre datos;
- ejemplos del capitulo 4 sobre reportes visuales.

## Parafrasis posible

Los datos abiertos no hablan por si solos: necesitan formas de presentacion que los vuelvan inteligibles y utiles.
